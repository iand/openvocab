...
class ARC2_RDFXMLParser extends ARC2_RDFParser {
  ...
  function h1Open($t, $a) {
    ...
    if ($t !== $this->rdf.'Description') {
      $this->addT($s['value'], $this->rdf.'type', $t, $s['type'], 'iri');
    }
    /* (additional) typing attr */
    if (isset($a[$this->rdf.'type'])) {
      $this->addT($s['value'], $this->rdf.'type', $a[$this->rdf.'type'], $s['type'], 'iri');
    }
    /* Seq|Bag|Alt */
  ...
  function h2Open($t, $a) {
    ...
    if (isset($a[$this->rdf.'resource'])) {
      ...
      /* type */
      if (isset($a[$this->rdf.'type'])) {
        $this->addT($o['value'], $this->rdf.'type', $a[$this->rdf.'type'], 'iri', 'iri');
      }
      /* reification */
      if (isset($s['p_id'])) {
  ...
  function h4Close($t) {/* empty p | pClose after cdata | pClose after collection */
    if ($s = $this->getParentS()) {
      ...
      if (isset($s['is_coll']) && $s['is_coll']) {
        $this->addT($s['value'], $this->rdf . 'rest', $this->rdf . 'nil', $s['type'], 'iri');
