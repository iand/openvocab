...
version:  2008-04-22 (Addition: unescapeNtripleUTF method)
*/
...
class ARC2_TurtleParser extends ARC2_RDFParser {
  ...
  function xCollection($v) {
    if ($sub_r = $this->x('\(', $v)) {
      ...
      do {
        $proceed = 0;
        if ((list($sub_r, $sub_v) = $this->xVarOrTerm($sub_v)) && $sub_r) {
          $r['triples'][] = array('type' => 'triple', 's' => $s, 'p' => $this->rdf . 'first', 'o' => $sub_r['value'], 's_type' => 'bnode', 'p_type' => 'iri', 'o_type' => $sub_r['type'], 'o_lang' => $this->v('lang', '', $sub_r), 'o_datatype' => $this->v('datatype', '', $sub_r));
          $proceed = 1;
        }
        elseif ((list($sub_r, $sub_v) = $this->xCollection($sub_v)) && $sub_r) {
          $r['triples'][] = array('type' => 'triple', 's' => $s, 'p' => $this->rdf . 'first', 'o' => $sub_r['id'], 's_type' => 'bnode', 'p_type' => 'iri', 'o_type' => $sub_r['type'], 'o_lang' => '', 'o_datatype' => '');
        ...
        if ($proceed) {
          if ($sub_r = $this->x('\)', $sub_v)) {
            $sub_v = $sub_r[1];
            $r['triples'][] = array('type' => 'triple', 's' => $s, 'p' => $this->rdf . 'rest', 'o' => $this->rdf . 'nil', 's_type' => 'bnode', 'p_type' => 'iri', 'o_type' => 'iri', 'o_lang' => '', 'o_datatype' => '');
            $closed = 1;
            $proceed = 0;
          }
          else {
            $next_s = $this->createBnodeID();
            $r['triples'][] = array('type' => 'triple', 's' => $s, 'p' => $this->rdf . 'rest', 'o' => $next_s, 's_type' => 'bnode', 'p_type' => 'iri', 'o_type' => 'bnode', 'o_lang' => '', 'o_datatype' => '');
            $s = $next_s;
          }
        }
      } while ($proceed);
    ...
    if (strlen($v) > 3000) {/* avoid regex choke */
      ...
      while (($pos < strlen($v)) && ($next_pos = strpos($v, $delim, $pos))) {
        ...
        if (strlen($sub_v) < 3000) {
          ...
          return array(array('value' => substr($v, strlen($delim), - strlen($delim)), 'type' => $types[$delim]), $rest);
        }
      }
    }
    else {
      if ($r = $this->x('\'\'\'((\x5c[\']{1,3}[^\']|[\']{1,2}[^\']|[\x5c][^\']|[^\'\x5c]|\\\t|\\\b|\\\n|\\\r|\\\f|\\\u|\\\U|\x5c\")*)\'\'\'', $v)) {
        return array(array('value' => $r[1], 'type' => 'literal_long1'), $r[3] . $rest);
      }
      if ($r = $this->x('\"\"\"((\x5c[\"]{1,3}[^\"]|[\"]{1,2}[^\"]|[\x5c][^\"]|[^\"\x5c]|\\\t|\\\b|\\\n|\\\r|\\\f|\\\u|\\\U|\x5c\')*)\"\"\"', $v)) {
        return array(array('value' => $r[1], 'type' => 'literal_long2'), $r[3] . $rest);
      }
      if ($r = $this->x('\'(([^\x27\x5c\xa\xd]|\\\t|\\\b|\\\n|\\\r|\\\f|\\\u|\\\U|\\\"|\x5c\')*)\'', $v)) {
        if (!preg_match('/\x5c$/', $r[1])) {
          return array(array('value' => $r[1], 'type' => 'literal1'), $r[3] . $rest);
        }
      }
      if ($r = $this->x('\"(([^\x22\x5c\xa\xd]|\\\t|\\\b|\\\n|\\\r|\\\f|\\\u|\\\U|\\\"|\x5c\'|\x5c\x5c)*)\"', $v)) {
        if (!preg_match('/\x5c$/', $r[1])) {
          return array(array('value' => $r[1], 'type' => 'literal2'), $r[3] . $rest);
        }
      }
    }
  ...
  function xNIL($v) {
    if ($r = $this->x('\([\x20\x9\xd\xa]*\)', $v)) {
      return array(array('type' => 'iri', 'value' => $this->rdf . 'nil'), $r[1]);
    }
    return array(0, $v);
  }
  ...
  function unescapeNtripleUTF($v) {
    ...
    foreach ($mappings as $in => $out) {
      $v = preg_replace('/([^\x5c])\x5c([' . $in . '])/', '\\1' . $out, $v);
    }
