...
class ARC2_RSSParser extends ARC2_LegacyXMLParser {
  ...
  function extractChannel($els) {
    $res = array($this->rdf . 'type' => array(array('value' => $this->rss . 'channel', 'type' => 'iri')));
    $res = array_merge($res, $this->extractProps($els, 'channel'));
    return array($res[$this->rss . 'link'][0]['value'] => $res);
  }
  
  function extractItem($els) {
    $res = array($this->rdf . 'type' => array(array('value' => $this->rss . 'item', 'type' => 'iri')));
  ...
  function extractProps($els, $container) {
    ...
    foreach ($els as $info) {
      ...
      if ($k) {
        ...
        $res[$k][] = array('value' => $v, 'type' => in_array($k, $this->dt_props) || !preg_match('/^[a-z0-9]+\:[^\s]+$/is', $v) ? 'literal' : 'iri');
      }
    }
    return $res;
  }
